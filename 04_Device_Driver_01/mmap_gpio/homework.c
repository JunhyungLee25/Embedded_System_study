#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <sys/mman.h>
#include <termios.h>

#define BCM2711_PERI_BASE 0xFE000000 
#define BLOCK_SIZE (4 * 1024)
#define PAGE_SIZE (4 * 1024)
#define GPIO_BASE (BCM2711_PERI_BASE + 0x200000) 

#define GPFSEL0 0x00 // GPIO 0-9
#define GPSET0  0x1c // LED ON
#define GPCLR0  0x28 // LED OFF
#define GPFSEL1 0x04 // GPIO 10-19
#define GPLEV0  0x34 // GPIO Pin Level 

static struct termios init_setting, new_setting;

void init_keyboard() {
    tcgetattr(STDIN_FILENO, &init_setting);
    new_setting = init_setting;
    new_setting.c_lflag &= ~ICANON;
    new_setting.c_lflag &= ~ECHO;
    new_setting.c_cc[VMIN] = 0;
    new_setting.c_cc[VTIME] = 0;
    tcsetattr(0, TCSANOW, &new_setting);
}

void close_keyboard() {
    tcsetattr(0, TCSANOW, &init_setting);
}

char get_key() {
    char ch = -1;
    if(read(STDIN_FILENO, &ch, 1) != 1) ch = -1;
    return ch;
}

int main(int argc, char **argv) {
    int data;
    int memfd;
    char *addr_gpio, ch;
    volatile unsigned int *gpio;
    int tmp, prev = 0;
    int flag = 0; // LED flag

    if (argc > 1) {
        printf("Guide: Only program name.\n");
        printf("Use: %s\n", argv[0]);
    }

    memfd = open("/dev/mem", O_RDWR | O_SYNC);
    if(memfd < 0) {
        printf("mem open fail\n");
        return -1;
    }

    addr_gpio = (char*)mmap(NULL, BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, memfd, GPIO_BASE);

    if(addr_gpio == MAP_FAILED) {
        printf("mmap fail\n");
        return -1;
    }

    gpio = (volatile unsigned int*)addr_gpio;
    gpio[GPFSEL0 / 4] &= ~(7 << 12); // 12-14 bit -> 001
    gpio[GPFSEL0 / 4] |= (1 << 12); // GPIO 4 Output

    gpio[GPFSEL1 / 4] &= ~(7 << 21); // 21-23 bit -> 000
    gpio[GPCLR0 / 4] = (1 << 4); // LED OFF
    printf("Default : LED OFF\n");
    printf("[q]: Exit program\n");

    init_keyboard();

    while(1) {
        if(get_key() == 'q') {
            gpio[GPSET0 / 4] |= (1 << 4);
            printf("shut down program.\n");
            break;
        }
        tmp = gpio[GPLEV0 / 4] & (1 << 17);
        if(tmp != prev) {
            if(tmp & (1 << 17)){
                flag = !flag;
                if(flag){
                    gpio[GPSET0 / 4] = (1<<4);
                    printf("Button Pressed : LED ON\n");        
                } else{
                    gpio[GPCLR0 / 4] = (1<<4);
                    printf("Button Pressed : LED OFF\n");        
                }
                usleep(50000); // Button Debouncing
            }
            prev = tmp;
        }
    }

    munmap(addr_gpio, BLOCK_SIZE);
    close_keyboard();
    close(memfd);
    return 0;
}

